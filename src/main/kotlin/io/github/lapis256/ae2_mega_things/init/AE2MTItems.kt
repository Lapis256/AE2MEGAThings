...
object AE2MTItems {
    ...
    fun drive(typeName: String, idPrefix: String, tierSupplier: Supplier<StorageTier>, housing: ItemLike, factory: MEGADISKDriveFactory) =
        tierSupplier.get().let { tier -> register("${tier.namePrefix.replace("m", "M")} $typeName DISK Drive", "${idPrefix}_disk_drive_${tier.namePrefix}") {
            factory(tier.componentItem(), housing, tier.kilobytes(), tier.idleDrain).also { DISK_DRIVES.add(it) }
        }}

    private fun megaItemDrive(tierSupplier: Supplier<StorageTier>) = drive("MEGA Item", "item", tierSupplier, MEGA_ITEM_DISK_HOUSING, ::ItemDISKDrive)
    private fun fluidDrive(tierSupplier: Supplier<StorageTier>) = drive("ME Fluid", "fluid", tierSupplier, FLUID_DISK_HOUSING, ::FluidDISKDrive)
    private fun megaFluidDrive(tierSupplier: Supplier<StorageTier>) = drive("MEGA Fluid", "fluid", tierSupplier, MEGA_FLUID_DISK_HOUSING, ::FluidDISKDrive)
}
